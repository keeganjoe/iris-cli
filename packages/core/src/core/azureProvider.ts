...
import { DEFAULT_AZURE_EMBEDDING_MODEL } from '../config/models.js';

/**
 * Azure OpenAI provider that implements the ContentGenerator interface
 * Maps Azure OpenAI API calls to Gemini-compatible interface
 */
export class AzureProvider implements ContentGenerator {
  private client: OpenAI;
  private model: string;
  private toolCallIdMap: Map<string, string> = new Map(); // functionName -> toolCallId

  constructor(apiKey: string, model: string, endpoint: string, apiVersion: string = '2024-06-01') {
    ...
    try {
      ...
      if (isJsonRequest) {
        if (request.config?.responseSchema) {
          // Use Azure OpenAI's structured outputs with the schema
  ...
  async generateContentStream(
    ...
    return this.convertStreamToGeminiFormat(stream as unknown as AsyncIterable<any>);
  }

  async countTokens(request: CountTokensParameters): Promise<CountTokensResponse> {
    // Azure OpenAI doesn't have a direct token counting API like Gemini
  ...
  async embedContent(request: EmbedContentParameters): Promise<EmbedContentResponse> {
    ...
    const response = await this.client.embeddings.create({
      model: request.model || DEFAULT_AZURE_EMBEDDING_MODEL,
      input: text.trim(),
    });
  ...
  private convertToOpenAIMessages(contents: (Content | string)[]): any[] {
    const messages: any[] = [];
  ...
  private async *convertStreamToGeminiFormat(
    ...
    try {
      ...
      console.error('Error in Azure OpenAI streaming:', error);
      throw error;
    }
